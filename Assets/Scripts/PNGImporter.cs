...
namespace AssemblyCSharp {
	public class PNGImporter {
		 ...
		 */
		private static Dictionary<System.Drawing.Color, string> readDictionary(string dictionaryFilename){
			var fStream = File.OpenRead(dictionaryFilename);
			var reader = new StreamReader(fStream, Encoding.ASCII);
			...
			while(null != (line = reader.ReadLine())) {
				...
				if (tokens.Length == 2) {
					...
					throw new IOException("Dictionary file format incorrect at line " + lineNum);
				}
			}
